...
class ScheduleTimelineView @JvmOverloads constructor(context:Context,attrs:AttributeSet?=null):View(context,attrs){
 ...
 override fun onDraw(c:Canvas){
  super.onDraw(c);val w=width.toFloat();val top=62f;val bottom=76f;val radius=7f
  ...
  for(t in boundaries){val m=minute(t);val xx=x(m,w);p.color=0xff666666.toInt();c.drawText(t,xx.coerceIn(32f,w-32f),54f,p);p.strokeWidth=2f;c.drawLine(xx,top-6,xx,top+6,p)}
  if(showSun){
   fun sun(m:Int?,icon:String){if(m==null)return;val xx=x(m,w);p.textSize=29f;p.color=0xffffa000.toInt();val label=icon+" "+("%02d:%02d".format(m/60,m%60));c.drawText(label,xx.coerceIn(45f,w-45f),103f,p)}
   sun(sunrise,"☀");sun(sunset,"☾")
  }
  if(showNow){val n=ZonedDateTime.now(ZoneId.of("Europe/Kyiv"));val m=n.hour*60+n.minute;val xx=x(m,w);p.color=0xff202020.toInt();p.strokeWidth=4f;c.drawLine(xx,top-10,xx,bottom+5,p);p.textSize=32f;c.drawText("%02d:%02d".format(n.hour,n.minute),xx.coerceIn(44f,w-44f),24f,p)}
 }
}